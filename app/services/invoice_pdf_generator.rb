...
class InvoicePdfGenerator
  ...
  def generate
    Prawn::Document.new do |pdf|
      ...
      # Invoice details
      pdf.text "Invoice Date: #{Date.current.strftime('%B %d, %Y')}"
      pdf.text "Payment Date: #{@payment.payment_date.strftime('%B %d, %Y')}"
      pdf.text "Invoice #: #{@payment.id.split('-').first.upcase}"
      pdf.move_down 20
      ...
      # Total
      pdf.text "Total Due: #{format_currency(@payment.amount)}", size: 14, style: :bold, align: :right

      pdf.move_down 30

      # Payment status
      pdf.text "Payment Status: #{@payment.status.titleize}"
      ...
      if @payment.status == 'completed'
        ...
        pdf.text "Please submit payment by the due date.", align: :center, style: :italic
      end
      pdf.move_down 10
